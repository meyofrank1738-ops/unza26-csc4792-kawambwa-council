...
3. **2025 Cross-File Conflict Resolution (The Mawaya Case Study):**  
   In 2025, projects were published across three overlapping PDF files: `UPDATED`, `APPROVED`, and `DEFERRED`. Several projects appeared in both the master list and the deferred list with contradictory statuses. For instance, in **Mawaya Ward**, community health and education projects appeared as deferred due to limited funding, but were listed under updated proposals. We implemented an **Authority Hierarchy** (`APPROVED` [Rank 3] > `UPDATED` [Rank 2] > `DEFERRED` [Rank 1]), retained the primary authority record, enriched any missing fields (amounts, ward details) from duplicate submissions, and flagged `data_conflict = True` for auditability.
